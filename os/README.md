...
| Script | What it does |
|--------|-------------|
| `retropie.sh` | Installs RetroPie (EmulationStation + RetroArch) via basic_install — run before `retropie-emulators.sh` |
...
| `retropie-nas.sh` | Mounts NAS ROM library to `~/RetroPie/roms` via fstab — requires `nas.sh` and `retropie.sh` |
| `brightness.sh` | Installs `ddcutil` for DDC/CI brightness control over HDMI — requires `i2c.sh` |
